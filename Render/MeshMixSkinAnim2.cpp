...
std::vector<MeshMixSkinAnim2::AnimationInfo> GetAnimationInfoListFromController(
    LPD3DXANIMATIONCONTROLLER controller,
    const std::wstring& filePath)
{
    std::vector<MeshMixSkinAnim2::AnimationInfo> infoList;
    ...
    for (UINT i = 0; i < animationSetCount; ++i)
    {
        ...
        MeshMixSkinAnim2::AnimationInfo info;
...
void MeshMixSkinAnim2::SetSharedMirrorClipPlane(const bool enabled, const D3DXVECTOR4& plane)
{
...
MeshMixSkinAnim2::~MeshMixSkinAnim2()
{
...
void MeshMixSkinAnim2::Initialize(bool async)
{
    if (m_bLoaded)
    {
        return;
    ...
    if (async)
    {
        ...
        m_loadThread = std::thread([this]() {
            ...
            {
                Util::LogMeshLoadFailure(L"MeshMixSkinAnim2", m_meshName, exception.what());
            }
            catch (...)
            {
                Util::LogMeshLoadFailure(L"MeshMixSkinAnim2", m_meshName, nullptr);
            }
        });
    }
    else
...
void MeshMixSkinAnim2::WaitForLoad()
{
    if (m_loadThread.joinable())
    {
        m_loadThread.join();
...
void MeshMixSkinAnim2::InitializeInternal()
{
    HRESULT hr = E_FAIL;
...
void MeshMixSkinAnim2::UpdateAnimation()
{
...
D3DXMATRIX MeshMixSkinAnim2::BuildWorldMatrix() const
{
...
void MeshMixSkinAnim2::InvalidateBonePaletteCache()
{
...
const std::vector<D3DXMATRIX>* MeshMixSkinAnim2::GetCachedBonePalette(const SkinAnimMeshContainer* container,
                                                                     DWORD paletteIndex)
{
...
void MeshMixSkinAnim2::ApplyAnimationFrameTransformsToMeshHierarchy(const LPD3DXFRAME meshFrameBase,
...
void MeshMixSkinAnim2::CollectUnknownAnimationFrames(const LPD3DXFRAME animationFrameBase,
...
void MeshMixSkinAnim2::ThrowIfAnimationHasUnknownFrames(const LPD3DXFRAME animationFrameRoot,
...
void MeshMixSkinAnim2::SetAnimationSpeed(const float speed)
{
    m_animationSpeed = speed;
    m_animController.SetAnimSpeed(speed);
}

bool MeshMixSkinAnim2::IsLoaded() const
{
    return m_bLoaded.load();
}

void MeshMixSkinAnim2::UpdateActiveAnimationClip()
{
...
bool MeshMixSkinAnim2::LoadAnimationCsv()
{
...
bool MeshMixSkinAnim2::LoadAnimationClip(const AnimationInfo& info)
{
    AnimationClip clip;
    clip.info = info;
...
HRESULT MeshMixSkinAnim2::LoadMeshHierarchy(const std::wstring& filePath,
...
HRESULT MeshMixSkinAnim2::LoadMeshHierarchyWithDirectX(const std::wstring& filePath,
                                                      SkinAnimMeshAlloc& allocator,
                                                      LPD3DXFRAME* frameRoot,
                                                      LPD3DXANIMATIONCONTROLLER* animationController)
{
...
HRESULT MeshMixSkinAnim2::LoadMeshHierarchyWithCustomLoader(const std::wstring& filePath,
                                                         SkinAnimMeshAlloc& allocator,
                                                         LPD3DXFRAME* frameRoot,
                                                         LPD3DXANIMATIONCONTROLLER* animationController,
                                                         XLoader::CustomXLoadPurpose loadPurpose)
...
void MeshMixSkinAnim2::ReleaseAnimationClips()
{
...
void MeshMixSkinAnim2::UpdateFrameMatrix(const LPD3DXFRAME frameBase, const LPD3DXMATRIX matParent)
{
    auto frame = reinterpret_cast<SkinAnimMeshFrame*>(frameBase);
...
void MeshMixSkinAnim2::RenderFrame(const LPD3DXFRAME frame)
{
...
void MeshMixSkinAnim2::RenderToEffect(LPD3DXEFFECT effect)
{
...
void MeshMixSkinAnim2::RenderFrameToEffect(const LPD3DXFRAME frame, LPD3DXEFFECT effect)
{
    LPD3DXMESHCONTAINER container = frame->pMeshContainer;
    while (container != nullptr)
    {
...
void MeshMixSkinAnim2::RenderMeshContainerToEffect(const LPD3DXMESHCONTAINER containerBase, LPD3DXEFFECT effect)
{
...
void MeshMixSkinAnim2::RenderMeshContainer(const LPD3DXMESHCONTAINER containerBase)
{
    auto container = reinterpret_cast<SkinAnimMeshContainer*>(containerBase);
    if (container->m_boneBuffer == nullptr)
    {
...
HRESULT MeshMixSkinAnim2::AllocateBoneMatrix(LPD3DXMESHCONTAINER containerBase)
{
...
HRESULT MeshMixSkinAnim2::AllocateAllBoneMatrix(LPD3DXFRAME frame)
{
    if (frame == nullptr)
    {
        return S_OK;
...
void MeshMixSkinAnim2::ReleaseMeshAllocator(const LPD3DXFRAME frame)
{
    ReleaseMeshAllocatorRecursive(frame, m_allocator);
}

void MeshMixSkinAnim2::ReleaseMeshContainersRecursive(const LPD3DXFRAME frame,
                                                     SkinAnimMeshAlloc& allocator)
{
    if (frame == nullptr)
    {
...
void MeshMixSkinAnim2::ReleaseMeshAllocatorRecursive(const LPD3DXFRAME frame, SkinAnimMeshAlloc& allocator)
{
    if (frame == nullptr)
    {
        return;
...
void MeshMixSkinAnim2::SetPos(const D3DXVECTOR3& pos)
{
    m_pos = pos;
    InvalidateBonePaletteCache();
}

void MeshMixSkinAnim2::SetSaturateShadow(const bool enabled)
{
    m_param.saturateShadow = enabled;
}

void MeshMixSkinAnim2::SetSaturateShadowIntensity(const float intensity)
{
    m_param.saturateShadowIntensity = intensity;
}

void MeshMixSkinAnim2::SetShadowDarkness(const float darkness)
{
    m_param.shadowDarkness = darkness;
}

void MeshMixSkinAnim2::SetSpecularIntensity(const float intensity)
{
    m_param.specularIntensity = intensity;
}

void MeshMixSkinAnim2::SetSpecularEdge(const float edge)
{
    m_param.specularEdge = edge;
}

void MeshMixSkinAnim2::SetFresnelIntensity(const float intensity)
{
    m_param.fresnelIntensity = (std::max)(0.0f, intensity);
}

void MeshMixSkinAnim2::SetSpecularIntensityOverrideEnabled(const bool enabled)
{
    m_param.specularIntensityOverrideEnabled = enabled;
}

void MeshMixSkinAnim2::SetSpecularEdgeOverrideEnabled(const bool enabled)
{
    m_param.specularEdgeOverrideEnabled = enabled;
}

void MeshMixSkinAnim2::SetTreatTextureAsWhite(const bool enabled)
{
    m_param.treatTextureAsWhite = enabled;
}

void MeshMixSkinAnim2::SetDamageFlash(const bool enabled)
{
    m_damageFlash = enabled;
}

void MeshMixSkinAnim2::SetYellowFlash(const bool enabled)
{
    m_yellowFlash = enabled;
}

void MeshMixSkinAnim2::SetCustomFlash(const bool enabled, const D3DXVECTOR4& color)
{
    m_customFlash = enabled;
    m_customFlashColor = color;
}

bool MeshMixSkinAnim2::GetBoneWorldMatrix(const char* boneName, D3DXMATRIX& outMatrix) const
{
    if (m_frameRoot == nullptr)
    {
        return false;
...
void MeshMixSkinAnim2::SetAlphaClipEnabled(const bool enabled)
{
    m_alphaClipEnabled = enabled;
}

void MeshMixSkinAnim2::SetIgnoreTransparentMaterial(const bool enabled)
{
    m_ignoreTransparentMaterial = enabled;
}

void MeshMixSkinAnim2::SetRot(const D3DXVECTOR3& rotate)
{
    m_rotate = D3DXVECTOR3(rotate.x, rotate.y + D3DX_PI, rotate.z);
    InvalidateBonePaletteCache();
}
...
void MeshMixSkinAnim2::SetScale(const float scale)
{
    m_scale = scale;
    InvalidateBonePaletteCache();
}
...
float MeshMixSkinAnim2::GetScale() const
{
    return m_scale;
}

bool MeshMixSkinAnim2::IsEnabled() const
{
    return m_enabled;
}

void MeshMixSkinAnim2::SetEnabled(const bool enabled)
{
    m_enabled = enabled;
}

std::wstring MeshMixSkinAnim2::GetMeshName() const
{
    return m_meshName;
}

const std::vector<MeshMixSkinAnim2::AnimationInfo>& MeshMixSkinAnim2::GetAnimationInfoList() const
{
    return m_animationInfoList;
}

bool MeshMixSkinAnim2::PlayAnimation(const std::wstring& name)
{
...
void MeshMixSkinAnim2::OnDeviceLost()
{
    const HRESULT hr = m_D3DEffect->OnLostDevice();
    assert(hr == S_OK);
}

void MeshMixSkinAnim2::OnDeviceReset()
{
