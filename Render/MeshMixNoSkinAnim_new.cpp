...
void MeshMixNoSkinAnim::SetSharedMirrorClipPlane(const bool enabled, const D3DXVECTOR4& plane)
{
...
MeshMixNoSkinAnim::MeshMixNoSkinAnim(const std::wstring& meshFilename,
...
void MeshMixNoSkinAnim::Initialize(bool async)
{
    if (m_bLoaded)
    {
        return;
...
void MeshMixNoSkinAnim::WaitForLoad()
{
    if (m_loadThread.joinable())
    {
        m_loadThread.join();
...
void MeshMixNoSkinAnim::InitializeInternal()
{
    HRESULT hr = E_FAIL;
...
void MeshMixNoSkinAnim::Render()
{
    ...
    const char* techniqueName = "TechniqueNoSkin";
    if (m_alphaClipEnabled)
    {
...
const std::vector<D3DXMATRIX>* MeshMixNoSkinAnim::GetCachedBonePalette(const NoSkinAnimMeshContainer* container,
                                                                     DWORD paletteIndex)
{
...
void MeshMixNoSkinAnim::ApplyAnimationFrameTransformsToMeshHierarchy(const LPD3DXFRAME meshFrameBase,
                                                                   const LPD3DXFRAME animationFrameRoot)
{
    ...
    auto meshFrame = reinterpret_cast<NoSkinAnimFrame*>(meshFrameBase);
    if (meshFrame->Name != nullptr)
    {
        ...
        {
            auto animationFrame = reinterpret_cast<NoSkinAnimFrame*>(animationFrameBase);
...
void MeshMixNoSkinAnim::SetAnimationSpeed(const float speed)
{
    m_animationSpeed = speed;
}

bool MeshMixNoSkinAnim::IsLoaded() const
{
...
bool MeshMixNoSkinAnim::LoadAnimationCsv()
{
...
bool MeshMixNoSkinAnim::LoadAnimationClip(const AnimationInfo& info)
{
    AnimationClip clip;
    clip.info = info;
    clip.loop = (info.mode == L"loop" || info.mode == L"default");
    clip.stopWhenEnd = (info.mode == L"stopWhenEnd");
    clip.allocator = NEW NoSkinAnimMeshAlloc(info.filePath);
...
HRESULT MeshMixNoSkinAnim::LoadMeshHierarchy(const std::wstring& filePath,
...
HRESULT MeshMixNoSkinAnim::LoadMeshHierarchyWithDirectX(const std::wstring& filePath,
                                                      NoSkinAnimMeshAlloc& allocator,
                                                      LPD3DXFRAME* frameRoot,
                                                      LPD3DXANIMATIONCONTROLLER* animationController)
{
...
HRESULT MeshMixNoSkinAnim::LoadMeshHierarchyWithCustomLoader(const std::wstring& filePath,
                                                         NoSkinAnimMeshAlloc& allocator,
                                                         LPD3DXFRAME* frameRoot,
                                                         LPD3DXANIMATIONCONTROLLER* animationController,
                                                         CustomXLoadPurpose loadPurpose)
...
void MeshMixNoSkinAnim::ReleaseAnimationClips()
{
...
void MeshMixNoSkinAnim::UpdateFrameMatrix(const LPD3DXFRAME frameBase, const LPD3DXMATRIX matParent)
{
    auto frame = reinterpret_cast<NoSkinAnimFrame*>(frameBase);
...
void MeshMixNoSkinAnim::RenderFrame(const LPD3DXFRAME frame)
{
...
void MeshMixNoSkinAnim::RenderToEffect(LPD3DXEFFECT effect)
{
...
void MeshMixNoSkinAnim::RenderFrameToEffect(const LPD3DXFRAME frame, LPD3DXEFFECT effect)
{
    LPD3DXMESHCONTAINER container = frame->pMeshContainer;
    while (container != nullptr)
    {
...
void MeshMixNoSkinAnim::RenderMeshContainerToEffect(const LPD3DXMESHCONTAINER containerBase, LPD3DXEFFECT effect)
{
    auto container = reinterpret_cast<NoSkinAnimMeshContainer*>(containerBase);
    if (container->m_boneBuffer == nullptr)
    {
        return;
    }
...
void MeshMixNoSkinAnim::RenderMeshContainer(const LPD3DXMESHCONTAINER containerBase)
{
    auto container = reinterpret_cast<NoSkinAnimMeshContainer*>(containerBase);
    if (container->m_boneBuffer == nullptr)
    {
...
HRESULT MeshMixNoSkinAnim::AllocateBoneMatrix(LPD3DXMESHCONTAINER containerBase)
{
    NoSkinAnimFrame *frame = nullptr;
    ...
    for (DWORD i = 0; i < boneCount; ++i)
    {
        ...
        frame = reinterpret_cast<NoSkinAnimFrame*>(p);
        if (frame == nullptr)
        {
            return E_FAIL;
        }
...
HRESULT MeshMixNoSkinAnim::AllocateAllBoneMatrix(LPD3DXFRAME frame)
{
    if (frame == nullptr)
    {
        return S_OK;
...
void MeshMixNoSkinAnim::ReleaseMeshAllocator(const LPD3DXFRAME frame)
{
    ReleaseMeshAllocatorRecursive(frame, m_allocator);
}

void MeshMixNoSkinAnim::ReleaseMeshContainersRecursive(const LPD3DXFRAME frame,
                                                     NoSkinAnimMeshAlloc& allocator)
{
    if (frame == nullptr)
    {
...
void MeshMixNoSkinAnim::ReleaseMeshAllocatorRecursive(const LPD3DXFRAME frame, NoSkinAnimMeshAlloc& allocator)
{
    if (frame == nullptr)
    {
        return;
...
void MeshMixNoSkinAnim::SetPos(const D3DXVECTOR3& pos)
{
    m_pos = pos;
    InvalidateBonePaletteCache();
}

void MeshMixNoSkinAnim::SetSaturateShadow(const bool enabled)
{
    m_param.saturateShadow = enabled;
}

void MeshMixNoSkinAnim::SetSaturateShadowIntensity(const float intensity)
{
    m_param.saturateShadowIntensity = intensity;
}

void MeshMixNoSkinAnim::SetShadowDarkness(const float darkness)
{
    m_param.shadowDarkness = darkness;
}

void MeshMixNoSkinAnim::SetSpecularIntensity(const float intensity)
{
    m_param.specularIntensity = intensity;
}

void MeshMixNoSkinAnim::SetSpecularEdge(const float edge)
{
    m_param.specularEdge = edge;
}

void MeshMixNoSkinAnim::SetFresnelIntensity(const float intensity)
{
    m_param.fresnelIntensity = (std::max)(0.0f, intensity);
}

void MeshMixNoSkinAnim::SetSpecularIntensityOverrideEnabled(const bool enabled)
{
    m_param.specularIntensityOverrideEnabled = enabled;
}

void MeshMixNoSkinAnim::SetSpecularEdgeOverrideEnabled(const bool enabled)
{
    m_param.specularEdgeOverrideEnabled = enabled;
}

void MeshMixNoSkinAnim::SetTreatTextureAsWhite(const bool enabled)
{
    m_param.treatTextureAsWhite = enabled;
}

void MeshMixNoSkinAnim::SetDamageFlash(const bool enabled)
{
    m_damageFlash = enabled;
}

void MeshMixNoSkinAnim::SetYellowFlash(const bool enabled)
{
    m_yellowFlash = enabled;
}

bool MeshMixNoSkinAnim::GetBoneWorldMatrix(const char* boneName, D3DXMATRIX& outMatrix) const
{
    if (m_frameRoot == nullptr)
    {
        return false;
    ...
    auto skinFrame = reinterpret_cast<NoSkinAnimFrame*>(frame);
    outMatrix = skinFrame->m_combinedMatrix;
    return true;
}

void MeshMixNoSkinAnim::SetAlphaClipEnabled(const bool enabled)
{
    m_alphaClipEnabled = enabled;
}

void MeshMixNoSkinAnim::SetIgnoreTransparentMaterial(const bool enabled)
{
    m_ignoreTransparentMaterial = enabled;
}

void MeshMixNoSkinAnim::SetRotY(const float rotY)
{
    m_rotate.y = rotY;
    InvalidateBonePaletteCache();
}

void MeshMixNoSkinAnim::SetScale(const float scale)
{
    m_scale = scale;
    InvalidateBonePaletteCache();
}

D3DXVECTOR3 MeshMixNoSkinAnim::GetRot() const
{
    return m_rotate;
}

D3DXVECTOR3 MeshMixNoSkinAnim::GetPos() const
{
    return m_pos;
}

float MeshMixNoSkinAnim::GetScale() const
{
    return m_scale;
}

bool MeshMixNoSkinAnim::IsEnabled() const
{
    return m_enabled;
}

void MeshMixNoSkinAnim::SetEnabled(const bool enabled)
{
    m_enabled = enabled;
}

std::wstring MeshMixNoSkinAnim::GetMeshName() const
{
    return m_meshName;
}

const std::vector<MeshMixNoSkinAnim::AnimationInfo>& MeshMixNoSkinAnim::GetAnimationInfoList() const
{
    return m_animationInfoList;
}

bool MeshMixNoSkinAnim::PlayAnimation(const std::wstring& name)
{
...
void MeshMixNoSkinAnim::OnDeviceLost()
{
    const HRESULT hr = m_D3DEffect->OnLostDevice();
    assert(hr == S_OK);
}

void MeshMixNoSkinAnim::OnDeviceReset()
{
