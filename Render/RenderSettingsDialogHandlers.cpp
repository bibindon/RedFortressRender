                      ...
                      int& skippedCount)
{
    ...
    while (std::getline(file, line))
    {
        ...
        try
        {
            ...
            int renderId = -1;
            RenderSettingsDialogState::LoadedModelType modelType = RenderSettingsDialogState::LoadedModelType::MeshMix2;
...
bool HandleRenderSettingsEditCommit(HWND hWnd, int id)
{
    ...
    if (id == 42143 && TryGetSettingsEditFloat(hWnd, id, floatValue))
    {
        render->SetDamageScale(floatValue);
        SetTrackbarFromFloat(hWnd, 32143, floatValue, 0.1f, 10.0f);
        return true;
    }
    return false;
}
    ...
    if (notifyCode == BN_CLICKED)
    {
        ...
        else if (id == 31331)
        {
                                           ...
                                           state->meshMixSkinAnimPath))
            {
                ...
                const int renderId = render->AddMeshMixSkinAnim2(state->meshMixSkinAnimPath,
                                                                 pos,
        ...
        else if (id == 31362)
        {
            if (!state->meshMixSkinNonAnimPath.empty() && !state->meshMixSkinAnimOnlyPath.empty())
            {
                ...
                D3DXVec3Normalize(&forward, &forward);
                const float yaw = atan2f(forward.x, forward.z);
                const int renderId = render->AddMeshMixSkinAnim2(state->meshMixSkinNonAnimPath,
                                                                 state->meshMixSkinAnimOnlyPath,
                                                                 pos,
                                                                 D3DXVECTOR3(0.0f, yaw, 0.0f),
                                                                 state->modelLoadScale,
...
void HandleRenderSettingsHScroll(HWND hWnd, LPARAM lParam)
{
    ...
    case 32142:
        ...
        break;
    case 32143:
        render->SetDamageScale(TrackbarToFloat(trackbar, 0.1f, 10.0f));
        SetEditFloat(hWnd, trackbar, TrackbarToFloat(trackbar, 0.1f, 10.0f), L"%.2f");
        break;
    default:
        break;
