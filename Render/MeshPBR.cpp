...
MeshPBR::MeshPBR(const std::wstring& xFilename,
...
void MeshPBR::ModifyMeshForNormalMapping(LPD3DXMESH& pMesh)
{
...
void MeshPBR::SetPos(const D3DXVECTOR3& pos)
{
    m_pos = pos;
}

void MeshPBR::SetSaturateShadow(const bool enabled)
{
    m_param.saturateShadow = enabled;
}

void MeshPBR::SetSaturateShadowIntensity(const float intensity)
{
    m_param.saturateShadowIntensity = intensity;
}

void MeshPBR::SetShadowDarkness(const float darkness)
{
    m_param.shadowDarkness = darkness;
}

void MeshPBR::SetSpecularIntensity(const float intensity)
{
    m_param.specularIntensity = intensity;
}

void MeshPBR::SetSpecularEdge(const float edge)
{
    m_param.specularEdge = edge;
}

void MeshPBR::SetSSS(const bool enabled)
{
    m_param.sss = enabled;
}

void MeshPBR::SetSSSIntensity(const float intensity)
{
    m_param.sssIntensity = intensity;
}

void MeshPBR::SetSSSColor(const DWORD color)
{
    m_param.sssColor = color;
}

void MeshPBR::SetRotY(const float rotY)
{
    m_rotate.y = rotY;
}

D3DXVECTOR3 MeshPBR::GetRot() const
{
    return m_rotate;
}

D3DXVECTOR3 MeshPBR::GetPos() const
{
    return m_pos;
}

float MeshPBR::GetScale() const
{
    return m_scale;
}

DWORD MeshPBR::GetSubsetCount() const
{
    return m_subsetCount;
}

bool MeshPBR::IsEnabled() const
{
    return m_enabled;
}

void MeshPBR::SetEnabled(const bool enabled)
{
    m_enabled = enabled;
}
...
LPD3DXMESH MeshPBR::GetD3DMesh() const
{
    return m_D3DMesh;
}

D3DXVECTOR4 MeshPBR::GetSubsetDiffuse(const DWORD subsetIndex) const
{
...
LPDIRECT3DBASETEXTURE9 MeshPBR::GetSubsetTexture(const DWORD subsetIndex) const
{
...
float MeshPBR::GetRadius() const
{
    return m_param.collisionRadius;
}

std::wstring MeshPBR::GetMeshName()
{
    return m_meshName;
}

void MeshPBR::OnDeviceLost()
{
    HRESULT hr = m_D3DEffect->OnLostDevice();
    assert(hr == S_OK);
}

void MeshPBR::OnDeviceReset()
{
    HRESULT hr = m_D3DEffect->OnResetDevice();
    assert(hr == S_OK);
}

stMeshPBRParam GetMeshPBRParamPreset(const eMeshPBRParamPreset preset)
{
