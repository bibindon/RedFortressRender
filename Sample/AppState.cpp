...
std::wstring g_selectedPbrEnvMapPath;
std::wstring g_selectedMeshInstancingPath;
bool g_bMeshMixHighQuality = false;
bool g_bMeshInstancingHighQuality = false;
bool g_bMeshMixSkinAnimAlphaClip = true;
...
std::wstring g_selectedSplitSkinNonAnimMeshPath;
std::wstring g_selectedSplitSkinAnimOnlyMeshPath;
...
float g_ssaoShadowStrength = 1.0f;
...
float g_pbrEnvReflectionIntensity = 0.05f;
float g_pbrEnvMaxMipLevel = 5.0f;
float g_pbrEnvDiffuseIntensity = 0.8f;
float g_pbrEnvDiffuseMipLevel = 3.0f;
...
float g_bloomThreshold = 0.95f;
float g_bloomWeightSum = 0.1f;
float g_haloThreshold = 1.5f;
...
float g_starBurstThreshold = 0.9f;
...
bool g_bSSAOCompositeGaussian3x3 = false;
int g_sunId = 0;
...
void LoadSampleSettingsFromCsv(const std::wstring& settingsCsvPath)
{
    ...
    while (std::getline(file, line))
    {
        ...
        try
        {
            ...
            else if (key == L"SSAOShadowStrength")
            {
                g_ssaoShadowStrength = std::stof(value);
            }
            else if (key == L"SSAOShadowSaturationBoost")
            {
                g_ssaoShadowSaturationBoost = std::stof(value);
            }
            else if (key == L"SSAOSampleCount")
            {
                g_ssaoSampleCount = std::stoi(value);
            }
            ...
            else if (key == L"SSAOCompositeGaussian3x3Enable")
            {
