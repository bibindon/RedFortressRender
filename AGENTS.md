...
- Blender's DirectX exporter writes an **empty `AnimationKey` block (0 keys)** for any channel (S/R/T) that has no f-curve on a partially-keyed bone (fully-unkeyed bones get static rest-pose keys instead). D3DX evaluates missing channels as identity, which collapses the bind pose (the Skeleton enemy's sideways walk + limbs-on-the-ground bug). `CustomXLoader2::CreateAnimationControllerFromParsedData` synthesizes a static key from the frame's bind-pose local matrix for empty channels. Note D3DX keyframed animation sets compose rotation keys as the **conjugate** of `D3DXMatrixRotationQuaternion`, so synthesized rotation keys must be conjugated. Regression tests: `EmptyAnimationChannelKeepsBindPose`, `SkeletonRealIdleKeepsHipsBindRotation` in `UnitTest1`.
- `LoadLemonTreeBlender512WithMeshInstancing2` crashes the vstest host in this environment even on unmodified code; it kills any full-suite run after the third test. Run remaining tests individually or with a filter.
